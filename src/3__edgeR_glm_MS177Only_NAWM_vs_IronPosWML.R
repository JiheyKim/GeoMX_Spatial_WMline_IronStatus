...
# Contrast for gNAWM vs. gWML_Lesion_1
contrast <- makeContrasts(WML_Lesion_1 - NAWM, levels = design)
qlf <- glmQLFTest(fit, contrast = contrast)
...
group_samples <- which(g %in% c("WML_Lesion_1", "NAWM"))
...
output_file <- "WML_NAWM_vs_IronPos_FDR005_wGeneName_CORRECTED.jpg"
...
write.csv(significant_genes_out, 
          file = "DEGs_WML_IronPos_vs_NAWM_FDR005.csv", 
          row.names = FALSE)
