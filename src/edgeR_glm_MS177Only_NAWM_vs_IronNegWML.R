...
# Contrast for gNAWM vs. gWML_Lesion_2
contrast <- makeContrasts(WML_Lesion_2 - NAWM, levels = design)
qlf <- glmQLFTest(fit, contrast = contrast)
...
group_samples <- which(g %in% c("WML_Lesion_2", "NAWM"))

# Ensure that group_samples are ordered so WML_Lesion_2 comes first
...
output_file <- "WML_NAWM_vs_IronNeg_FDR005_wGeneName_CORRECTED.jpg"
...
significant_genes_out <- significant_genes %>%
  mutate(Gene = rownames(significant_genes)) %>%
  select(Gene, everything())

write.csv(significant_genes_out, 
          file = "DEGs_WML_IronNeg_vs_NAWM_FDR005.csv", 
          row.names = FALSE)
